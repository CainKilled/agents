...
# Run a subset of the test suite. Additional stubs or packages may be required.
# PYTHONPATH ensures our local stubs and sitecustomize are found.
PYTHONPATH=. python -m pytest -k test_tool_choice_options -q >test.log 2>&1 || {
    tail -n 20 test.log
    exit 1
}
